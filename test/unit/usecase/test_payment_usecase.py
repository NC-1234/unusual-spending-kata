...
def test_add_payment_for_user_should_add_payment_for_a_given_month(mock_credit_repository, mock_email_service, in_memory_payment_usecase):
    ...
    in_memory_payment_usecase.add_payment(TEST_USER_ID, payment=GOLF_PAYEMENT, month=MonthStatus.CURRENT)
    ...
    result = in_memory_payment_usecase.fetch_payment(TEST_USER_ID, MonthStatus.CURRENT)

    assert result == expected_expenditure
    assert type(result) == MonthlyExpenditure

def test_fetch_payment_for_user_should_return_current_monthly_expenditure(mock_credit_repository, mock_email_service, in_memory_payment_usecase):
    expected_expenditure = MonthlyExpenditure(month_status=MonthStatus.CURRENT,
                                              payments=[GOLF_PAYEMENT]
                                              )
    mock_credit_repository.fetch_payments_by_month.return_value = expected_expenditure
...
def test_return_all_categories_which_a_user_has_over_spent_by_50_per_cent(mock_credit_repository, mock_email_service, in_memory_payment_usecase):
    ...
    current_month = MonthlyExpenditure(month_status=MonthStatus.CURRENT,
                                       payments=[GOLF_PAYEMENT,
                                                 GOLF_PAYEMENT])
    result = in_memory_payment_usecase.calculate_over_spending(
...
def test_compose_email_should_create_an_email_for_user_overspent_categories(mock_credit_repository, mock_email_service, in_memory_payment_usecase):
    ...
    * You spent $999.99 on golf
    ...
    result = in_memory_payment_usecase.compose_email(test_overspent_categories)
